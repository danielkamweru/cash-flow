      ...
      <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex shrink-0 gap-3 text-[11px] text-cf-muted">
          <span className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full bg-cf-success" /> Inflow
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full bg-cf-primary" /> Outflow
          </span>
        </div>
      </div>
          ...
          {series.map((s, i) => (
            <div key={s.month} className="flex min-w-[2.25rem] flex-1 flex-col items-center gap-2">
              <div className="flex h-36 w-full items-end justify-center gap-0.5 sm:gap-1">
                <div
                  className="w-[42%] rounded-t-md bg-cf-success/80 transition-all duration-700"
                  style={{ height: `${(s.inflow / max) * 100}%`, animationDelay: `${i * 40}ms` }}
                  title={`In ${formatKes(s.inflow)}`}
                />
                <div
                  className="w-[42%] rounded-t-md bg-cf-primary/80 transition-all duration-700"
                  style={{ height: `${(s.outflow / max) * 100}%` }}
                  title={`Out ${formatKes(s.outflow)}`}
                />
              </div>
              <span className="text-[10px] text-cf-muted">{s.month}</span>
            </div>
          ))}
        </div>
      </div>
...
export function NetWorthHero({
  ...
  return (
    <section className="cf-card relative overflow-hidden p-4 sm:p-5 md:p-7 animate-fade-up">
      <div className="absolute inset-y-0 right-0 w-1/2 bg-gradient-to-l from-cf-primary/15 to-transparent" />
      <div className="relative min-w-0">
        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-cf-muted">
          Where am I?
        </p>
        ...
        <div className="mt-6 grid grid-cols-2 gap-2 sm:gap-3 md:grid-cols-4">
          {[
            ...
            <div
              key={m.label}
              className="min-w-0 rounded-xl border border-cf-border bg-[var(--cf-inset)] px-2.5 py-3 sm:px-3"
            >
